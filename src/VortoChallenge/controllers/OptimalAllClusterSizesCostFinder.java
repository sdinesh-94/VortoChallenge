...
public class OptimalAllClusterSizesCostFinder {
	...
	ClusterConfigCostCalculator clusterConfigCostCalcualtor; // algorithm used to calculate individual cluster config
																// costs
	DistanceMatrix dm;
	ArrayList<TripLoad> trips;
	int tripSize = 0;

	public void setupCostFinder(ClusterConfigCostCalculator clusterConfigCostCalculator, DistanceMatrix dm,
			ArrayList<TripLoad> trips) {
	...
	/*
	 * Calculates and returns the cost and the cluster configuration for a cluster of size 1 
	 */
	...
	/*
	 *  Calculates the minimal cost for n=1 to clusterSize. Once found it returns the minimal cost and 
	 *  clustersize and the optimal configuration that resulted in the cluster size
	 */
	ClusterConfigCost calculateMultipleClustersLowestCostCfg(int clusterSize) {
		...
		multipleClusterCfgCost = clusterConfigCostCalcualtor.calculateMultipleClusterCostCfg(trips, tripSize,
				clusterSize);
		return multipleClusterCfgCost;
	}

	/*
	 *  Core logic behind finding the optimal cost. We check for a global minima ( we assume that global minima
	 *  while be when cost to the immediate left is higher and cost to immediate is higher than the current point)
	 *  
	 */
	public ArrayList<ArrayList<Integer>> calculateOptimalCost() {
		...
		while (clusterSize < tripSize) {
			...
			if (minCostkCluster < minCostkminus1Cluster && minCostkCluster < minCostkplus1Cluster) { // k is global
																										// minima
