...
public class OptimalKClusterConfigCostCalculator extends ClusterConfigCostCalculator {
	...
	/*
	 * logic to calculate minimum cost among the most optimal cluster of size 2...clusterSize
	 */
	public ClusterConfigCost calculateMultipleClusterCostCfg(ArrayList<TripLoad> trips, int tripSize, int clusterSize) {
		...
		/*
		 * given cluster assignment for each trip in trips list, we convert to binary
		 * string representation for that cluster and calculate the associated cost
		 */
		...
		for (ArrayList<Integer> tripPlacementClusterIdx : alltripPlacementClusterIdx) {
			...
			ArrayList<String> clusterStringList = Cluster.getClusterStringRepresentation(tripPlacementClusterIdx,
					clusterSize);
    ...
    /*
     * recursive function to generate all combinations of arranging trips in different clusters
     */
	static ArrayList<ArrayList<Integer>> generatePlacementListRecFun(int tripSize, int tripPos, int clusterSize) {
		...
		ArrayList<ArrayList<Integer>> alltripPlacementClusterPrevIdx = generatePlacementListRecFun(tripSize,
				tripPos + 1, clusterSize);
		...
		for (ArrayList<Integer> alltripPlacementClusterPrevIdxEle : alltripPlacementClusterPrevIdx) {
			for (int indexToAppend = 1; indexToAppend <= clusterSize; indexToAppend++) {
				ArrayList<Integer> alltripPlacementClusterPrevIdxEleCopy = new ArrayList<>(
						alltripPlacementClusterPrevIdxEle);
	...
	/*
	 * logic to calculate singleClusterCost (only one driver). we consider all trips in same configuration
	 */
	@Override
	public ClusterConfigCost calculateSingleClusterCostCfg(ArrayList<TripLoad> trips, int tripSize) {
		...
		// classic TSP problem. To avoid exponential time complexity, we just go with
